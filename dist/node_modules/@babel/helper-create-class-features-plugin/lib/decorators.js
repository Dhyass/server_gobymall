...
function replaceClassWithVar(path, className) {
  ...
  if (path.type === "ClassDeclaration") {
    var _className = id.name;
    var varId = scope.generateUidIdentifierBasedOnNode(id);
    var classId = _core.types.identifier(_className);
    scope.rename(_className, varId.name);
  ...
  if (hasElementDecorators) {
    ...
    try {
      for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
        ...
        var _hasDecorators = !!(decorators != null && decorators.length);
        ...
        if (_isComputed) {
          if (!_element.get("key").isConstantExpression()) {
            var _key = _node.key;
            var _maybeAssignment = (0, _misc.memoiseComputedKey)(_hasDecorators ? createToPropertyKeyCall(state, _key) : _key, scopeParent, scopeParent.generateUid("computedKey"));
        ...
        if (_hasDecorators && version === "2023-11") {
      ...
      if (isAnonymous(initializer)) {
        if (!node.computed) {
          if (id.type === "Identifier") {
            ...
            var _className2 = _core.types.stringLiteral(id.value + "");
            visitor(initializer, state, _className2);
          }
        } else {
    ...
    visitor: Object.assign({
      ExportDefaultDeclaration: function ExportDefaultDeclaration(path, state) {
        ...
        if ((declaration == null ? void 0 : declaration.type) === "ClassDeclaration" && isDecorated(declaration)) {
          ...
          var updatedVarDeclarationPath = path.splitExportDeclaration();
          if (isAnonymous) {
